...
工具可用轮采用 `buffered-final`：工具进度实时到达，最终正文通常在一个 `answer.delta` 事件中完整返回。仅在服务端工具、轮数或工具结果预算耗尽后，可能出现禁用工具的收束轮；该轮的 `answer.delta` 使用 `streaming-final` 并是真实增量。客户端始终应按增量追加正文。
