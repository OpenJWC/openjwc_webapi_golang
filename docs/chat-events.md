...
瞬时模型故障（网络失败、429、5xx、流提前截断）会自动重试一次：等待数秒（尊重有界的 Retry-After）、不超过剩余运行预算；鉴权错误和协议错误不重试，收束轮一旦已发布文字也不再重试。服务端日志会记录脱敏的失败细节（如 `upstream_429`、`stream_truncated`、`finish_length`、`empty_content`）与上游状态码，不包含问题、答案或供应商响应内容。
...
- `date` 返回配置时区的当前日期时间（RFC3339），帮助模型判断“今天/昨天”等自然日；不接受参数，也不能参与管道。
