...
- Agent 提供同步事件消费端口，HTTP 使用八事件队列与写入期限施加背压。工具过程即时公开；工具轮的模型正文从不公开，所有最终回答都经一次禁用工具的流式轮生成，以 streaming-final 逐字转发，仅当供应商不支持 SSE 时才回退 buffered-final。旧适配器仅收集答案，两条 chat 路由共享用户并发限制。
